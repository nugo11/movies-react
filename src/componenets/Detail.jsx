...
export default function Detail() {
  ...
  return (
    <>
      <div className="fullbg" style={{backgroundImage: `url(../src/db/${selectedItem.poster})`}}></div>
  ...
  const isLastIndex = index === selectedItem.movieScriptContent.length - 1;
  const isSecondToLastIndex = index === selectedItem.movieScriptContent.length - 2;

  if (selectedItem.country.includes('ინგლისურად') && selectedItem.country.includes('რუსულად')) {
    if (isLastIndex) {
      buttonText = 'რუსულად';
    } else if (isSecondToLastIndex) {
      buttonText = 'ინგლისურად';
    }
  } else if (selectedItem.country.includes('ინგლისურად') && isLastIndex) {
    buttonText = 'ინგლისურად';
  } else if (selectedItem.country.includes('რუსულად') && isLastIndex) {
    buttonText = 'რუსულად';
  }
  ...
  return (
    <li className="nav-item" role="presentation" key={index}>
      <button
        id={`${index}-tab`}
        className={`${count === index ? "active" : "activei"}`}
        data-bs-toggle="tab"
                    ...
                    <div className="col-12 col-md-7 col-lg-8 col-xl-6 col-xxl-7">
                      <div className="item__content">
                        <ul className="item__meta">
                          <li>
                            <span>წელი:</span> {selectedItem.year}
                          </li>
                          <li>
                            <span>ჟანრი:</span>
                            {selectedItem.genre.map((item, index) => (
                              <a key={index} href="catalog.html">
                                {item}
                              </a>
                            ))}
                          </li>
